...
(function($) {
    // Podemos definir que clases usare para la parte front y back de las cartas
    var front = 'flip_front',
        back = 'flip_back';
    var flip = function($dom, callback) {
        ...
        //Funcion para ejercicios toggle solo se permite una carta volteada, se activa con: data-toggle-flip ="true";
        if ($dom[0].dataset.toggleFlip == 'true') {
            $($dom[0]).blur(function() {
                setTimeout(() => {
                    unflip($dom, callback);
                }, 1000);
    ...
    $.fn.flip = function(options, callback) {
        ...
        this.each(function() {
            ...
            if (
                ...
                var rotateAxis =
                        'rotate' + (settings.axis.toLowerCase() == 'x' ? 'x' : 'y'),
                    perspective =
                        $dom['outer' + (rotateAxis == 'rotatex' ? 'Height' : 'Width')]() *
                        2;
                ...
                $dom.find($dom.data(back)).css({
                    transform:
                        rotateAxis +
                        '(' +
                        (settings.reverse ? '180deg' : '-180deg') +
                        ')',
                });
    ...
    var changeSettings = function(options, callback) {
        ...
        if (changeNeeded) {
            var faces = $(this)
                .find($(this).data(front))
                .add($(this).data(back), $(this));
            ...
            if ($(this).data('flipped')) {
                $(this)
                    ...
                    .css({
                        transform:
                            rotateAxis +
                            ($(this).data('reverse') ? '(-180deg)' : '(180deg)'),
                        'z-index': '0',
                    });
            } else {
                $(this)
