...
function Play() {
    ...
    mode = 0;
    alert('PLAY');
    clueP = document.querySelectorAll('.clueP');
    clueP.forEach((element) => {
        console.log(element);
    });
...
function AddWordToBoard() {
    ...
    if (wordsActive.length < 1) {
        ...
        console.log('parece ser el bueno');
                ...
                for (var k = 0, lenK = wordsActive.length; k < lenK; k++) {
                    ...
                    for (var l = 0, lenL = testWord.char.length; l < lenL; l++) {
                        ...
                        if (curChar === testChar) {
                            ...
                            for (
                                var m = -1, lenM = curWord.char.length + 1;
                                m < lenM;
                                m++
                            ) {
                                var crossVal = [];
                                if (m !== j) {
                                    if (curCross.dir === 0) {
                                        ...
                                        crossVal.push(board[xIndex][curCross.y].value);
                                        crossVal.push(
                                            board[xIndex][curCross.y + 1].value
                                        );
                                        crossVal.push(
                                            board[xIndex][curCross.y - 1].value
                                        );
                                        ...
                                        if (
                                            yIndex < 0 ||
                                            yIndex > board[curCross.x].length
                                        ) {
                                            isMatch = false;
                                            break;
                                        }
                                        ...
                                        crossVal.push(board[curCross.x][yIndex].value);
                                        crossVal.push(
                                            board[curCross.x + 1][yIndex].value
                                        );
                                        crossVal.push(
                                            board[curCross.x - 1][yIndex].value
                                        );
...
function BoardCharToElement(c) {
    ...
    if (c.value !== null) {
        ...
        inner = EleStr(
            ...
            EleStr('span', [{ a: 'class', v: [`num ${num}`] }], num)
        );
    }
...
function BoardCharFocus(boardChar) {
    ...
    this.onkeypress = function(key) {
        ...
        console.log('asi llega: ', indexWord);
        console.log(boardChar.char[focusIndex]);

        if (boardChar.char[focusIndex].prev == null) {
            indexWord = document.activeElement;
            indexWord = indexWord.nextSibling.innerHTML;
            indexWord = parseInt(indexWord);
            indexWord += 1;
            console.log('Asi queda', indexWord);
        }

        if (boardChar.char[focusIndex].next !== null) {
            focusChar = boardChar.char[focusIndex].next;
            Comparative(focusChar, key.key, idElement);
            document
                .getElementsByClassName('char')
                [boardChar.char[focusIndex].next.index].focus();
        } else {
            console.log('se acabo la palabra.', indexWord);

            let newFocusChar = document.getElementsByClassName(`${indexWord}`);

            console.log(newFocusChar);

            console.log(newFocusChar);
            focusChar = boardChar.char[focusIndex].prev;
            Comparative(focusChar, key.key, idElement);
            newFocusChar = newFocusChar[0].previousElementSibling
                ? newFocusChar[0].previousElementSibling
                : '';
            newFocusChar.focus();
        }
    };
...
function RegisterEvents() {
    ...
    document.getElementById('btnValidate').addEventListener('click', Validate, false);
}
RegisterEvents();
...
function Comparative(focusChar, key, idElement) {
    let valueSiguiente = focusChar.next;
    arrayValoresCom.push(key.toUpperCase());
    indice = arrayValores.length;
    arrayValores.push(idElement);

    if (arrayValores[indice] == arrayValoresCom[indice]) {
        document.activeElement.setAttribute('class', 'char letter-Right');
    } else {
        document.activeElement.setAttribute('class', 'char letter-Wrong');
    }
}

function Validate() {
    let classGood, classBad;
    classGood = document.getElementsByClassName('letter-Right');
    classBad = document.getElementsByClassName('letter-Wrong');
    if (classGood) {
        for (var i = 0; i < classGood.length; i++) {
            classGood[i].classList.add('bien');
        }
    }
    if (classBad) {
        for (var i = 0; i < classBad.length; i++) {
            classBad[i].classList.add('mal');
        }
    }
}
