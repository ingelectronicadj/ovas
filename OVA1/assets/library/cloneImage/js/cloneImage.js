...
const cloneImg_general = {
    ...
    assign_image: (e, waitImg) => {
        ...
        if (imgElement.nodeName === 'IMG') {
            if (waitImg.parentElement.getAttribute('texto') == null) {
                waitImg.setAttribute('aria-label', imgElement.alt);
            } else {
            ...
            console.log(
                `Valor para: waitImg`,
                waitImg.parentElement.getAttribute('texto')
            );
            console.log(`Valor para: waitImg- Padre`, waitImg.parentElement);
    ...
    validate_image: (elm_li) => {
        ...
        if (
            ...
            if (elm_li.getAttribute('texto') == null) {
                textImg.setAttribute(
                    'aria-label',
                    `Respuesta correcta. ${textImg.getAttribute('alt')}`
                );
            } else {
                textImg.setAttribute(
                    'aria-label',
                    `Respuesta correcta. ${elm_li.getAttribute(
                        'texto'
            ...
            if (elm_li.getAttribute('texto') == null) {
                textImg.setAttribute(
                    'aria-label',
                    `Respuesta incorrecta. ${textImg.getAttribute('alt')}`
                );
            } else {
                textImg.setAttribute(
                    'aria-label',
                    `Respuesta incorrecta. ${elm_li.getAttribute(
                        'texto'
                    )}. ${textImg.getAttribute('alt')}`
                );
            }
