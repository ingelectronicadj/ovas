...
class sendConveyorBelt{
    ...
    CreateObject(){
        ...
        Object.setAttribute('arialabel',`Elemento ${this.objectArray[i][0]}`)
    ...
    ResetPackage(box){
        ...
        let items = document.querySelectorAll('div[belt="'+box+'"]');
            ...
            if(valueLeft === 70){
            clearInterval(intervalo);
            ariaLiveAssertive.innerHTML = `La banda transportadora ${box} esta de nuevo lista para hacer otro pedido `;
    ...
    VerifyBox(box){
        ...
        temporalArray.forEach(function(item){
            let namesArray = document.querySelector('div[numberid="'+item+'"]')
            let itemName = namesArray.id;
    ...
    VerifyMission(box,idMission){
        ...
        let type;
        let correctSensor, correctController, correctActor,alternativecorrectActor,findActor,findSensor;
        let amountInItem = [];
        ...
        switch (tipocaso) {
                ...
                if(item[2] === currentObjectArray[i]){
                    ...
                    switch (type) {
                        case "Elementos de Entrada":
                            if(currentObjectArray[i] === SendConveyorGame.missionArray[idMission][1][0][0]){
                                correctSensor = true
                                }
                            else{correctSensor = false;}
                        break;
                        case "Elementos de Salida":
                            if(currentObjectArray[i] === SendConveyorGame.missionArray[idMission][1][2][0]){
                            correctActor = true;
                            }
                            else{correctActor = false;}
                        break;
            ...
            for(let i = 0; i < currentObjectArray.length; i++){
                ...
                if(item[2] === currentObjectArray[i]){
                    ...
                    switch (type) {
                        ...
                        case "Elementos de Salida":
                            ...
                            else if(currentObjectArray[i] === SendConveyorGame.missionArray[idMission][1][2][1]){
                ...
                if(correctActor && correctController && correctSensor && !alternativecorrectActor){
                    feedback.messages = {
                        bien: [SendConveyorGame.feedbackArray[idMission][0]],
                        mal: [''],
                    }
                ...
                else if(alternativecorrectActor && correctController && correctSensor && !correctActor){
                    feedback.messages = {
                ...
                else if((correctActor || alternativecorrectActor) && correctController && !correctSensor){
                    feedback.messages = {
                        bien: [''],
                        mal: [SendConveyorGame.feedbackArray[idMission][2]],
                    }
            ...
            for(let i = 0; i < currentObjectArray.length; i++){
                ...
                if((correctActor || alternativecorrectActor) && correctController && correctSensor){
                    feedback.messages = {
                        bien: [SendConveyorGame.feedbackArray[idMission][0]],
                        mal: [''],
                    }
            ...
            case "Caso5":
                for(let i = 0; i < currentObjectArray.length; i++){
                    ...
                    if(item[2] === currentObjectArray[i]){
                        ...
                        switch (type) {
                            case "Elementos de Entrada":
                                findSensor = SendConveyorGame.missionArray[idMission][1][0].every(element => currentObjectArray.indexOf(element) > -1);
                            break;
                            case "Elementos de Salida":
                                if(amountInItem.length > 1){
                                    findActor = SendConveyorGame.missionArray[idMission][1][2].every(element => currentObjectArray.indexOf(element) > -1);
                                }
                                else{
                                    if(currentObjectArray[i] === SendConveyorGame.missionArray[idMission][1][2][0]){
                                        correctActor = true
                            ...
                            break;
                            case "Elementos de Control":
                                if(SendConveyorGame.missionArray[idMission][1][1].length > 1){
                                    correctController = true; 
                                }
                                else{
                                if(currentObjectArray[i] === SendConveyorGame.missionArray[idMission][1][1][0]){
                                    correctController = true
                                    }
                                else{correctController = false;
                                }
                            break;
                            }
                    }
                    ...
                    else if(findSensor && correctController && !findActor && !correctActor){
                        feedback.messages = {
                            bien: [''],
                            mal: [SendConveyorGame.feedbackArray[idMission][3]],
                        }
                    ...
                    for(let i = 0; i < currentObjectArray.length; i++){
                        ...
                        if(item[2] === currentObjectArray[i]){
                            ...
                            switch (type) {
                                ...
                                break;
                                case "Elementos de Control":
                                    if(currentObjectArray[i] === SendConveyorGame.missionArray[idMission][1][1][1][0] || currentObjectArray[i] === SendConveyorGame.missionArray[idMission][1][1][1][1]){
                                        correctController = false
                                        }
                                    else{correctController = true;
                                    }
                                break;
                                }
                        }
                    ...
                    case "Caso6":
                        for(let i = 0; i < currentObjectArray.length; i++){
                          ...
                          if(findActor && correctController && findSensor){
                            ...
                            else if(findActor && correctController && (!correctSensor || !findSensor)){
                                feedback.messages = {
                                    bien: [''],
                                    mal: [SendConveyorGame.feedbackArray[idMission][2]],
                                }
